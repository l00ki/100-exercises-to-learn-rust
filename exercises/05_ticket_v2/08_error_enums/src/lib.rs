...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(t) => t,
        Err(TicketNewError::Title(e)) => panic!("{}", e),
        Err(TicketNewError::Description(_)) => Ticket::new(title, "Description not provided".to_string(), status).unwrap()
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::Title("Title cannot be longer than 50 characters".to_string()));
        }
        if description.is_empty() {
            return Err(TicketNewError::Description("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::Description("Description cannot be longer than 500 characters".to_string()));
        }
